...
if os.path.exists("index.yml"):
    ...
    for plugin in index['plugins']:
        if plugin['name'] == 'ImageStreamPlugin':
            ...
            break
    if not_present:
        index['plugins'].append({"name": "ImageStreamPlugin", "path": "https://github.com/%s/crane-plugins/raw/main/plugins/ImageStream/index.yaml"%sys.argv[1]})
...
if os.path.exists('plugins/ImageStream/index.yml'):

    file = open("plugins/ImageStream/index.yml","r")
    ...
    file = open("plugins/ImageStream/index.yml","w")

    yaml.dump(index, file)
    file.close()
    
else:
    file = open("plugins/ImageStream/index.yml","a+")
